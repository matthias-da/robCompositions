...
#' @references (Egozcue, J.J., Pawlowsky-Glahn, V. (2005) Groups of parts and their balances in compositional data analysis. Mathematical Geology, 37 (7), 795ñ828.)
#' @examples
...
#' b1 <- bal(expenditures, y1)
#' b2 <- bal(expenditures, y5)
#' b1$balances
#' b2$balances
#' 
...
#'                  c(0,0,+1,-1))
#' bal(machineOperators, sbp)
#' 
#' data(phd)
#' bal(phd,y1) # Error non-matching size
#' bal(phd[, 7:ncol(phd)], y1) # Error non-matching size
#' bal_phd<-bal(phd[, 4:ncol(phd)], y1) # OK
#' b1$balances
#' b1$V
#' 
