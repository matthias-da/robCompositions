...
#' t_step = diff(t[1:2])
#' mean = 0; sd = 1.5
#' dens.true = dnorm(t, mean, sd)
#' dens.true.l = fcenLR(t,t_step,dens.true)
#' trapzc(t_step,dens.true.l)
...
#' @param z grid of point defining the abscissa
...
#' dens.true = dnorm(t, mean, sd)
#' 
#' plot(t,dens.true, type="l",las=1, ylab="fcenLR(density)", 
#'   cex.lab=1.2,cex.axis=1.2, col="darkblue",lwd=2)
#' abline(h=0, col="red")
#' 
#' plot(t,fcenLRinv(t.fine,t_step,dens.true), type="l",las=1, 
#'   ylab="density",cex.lab=1.2,cex.axis=1.2, col="darkblue",lwd=2)
...
#' @param p = density of reference measure P
#' @export
#' @return 
...
#' @export
#' @return 
#' \item{\code{out}}{grid evaluation of the P-density in inweighted L2(lambda)}
#' @references Talska, R., Menafoglio, A., Hron, K., Egozcue, J. J., Palarea-Albaladejo, J. (2020). Weighting the domain of probability densities in functional data analysis.\emph{Stat}(2020). https://doi.org/10.1002/sta4.283
#' @examples
...
#' fp = (f/p_ref)/trapzc(t_step,f/p_ref)
#' 
...
#'   main=expression(bold(atop(paste('(a) Representation of f 
#'   in ', L^2, (lambda)),'[not weighted]'))))
#' abline(h=0,col="red")
#' 
#' plot(t, f, type='l', ylab=expression(f[lambda]), 
#'   xlab='t',las=1,ylim=c(0,0.4),
#'   main=expression(bold(atop(paste('(b) Density f 
#'     in ', B^2, (lambda)),'[not weighted]'))))
#' 
#' plot(t, fp, type='l', ylab=expression(f[P]), xlab='t',
#'   las=1,ylim=c(0,0.4),
#'   main=expression(bold(atop(paste('(c) Density f 
#'   in ', B^2, (P)),'[weighted with P]'))))
#' 
#' plot(t, fp.fcenLRp, type='l', ylab=expression(fcenLR[P](f[P])), 
#'   xlab='t',las=1,ylim=c(-3,3), 
#'   main=expression(bold(atop(paste('(d) Representation of f in ', 
#'     L^2, (P)),'[weighted with P]'))))
#' abline(h=0,col="red")
#' 
#' plot(t, fp.u, type='l', ylab=expression(paste(omega^(-1),(f[P]))), 
#'   xlab='t',las=1,ylim=c(0,0.4), 
#'   main=expression(bold(atop(paste('(e) Representation of f in ', 
#'   B^2, (lambda)),'[unweighted]'))))
#' 
#' plot(t, fp.fcenLRu, type='l', ylab=expression(paste(fcenLR[u](f[P]))), 
#'   xlab='t',las=1,ylim=c(-3,3),
#'   main=expression(bold(atop(paste('(f) Representation of f in ', 
#'   L^2, (lambda)),'[unweighted]'))))
#' abline(h=0,col="red")
...
#'   cex.lab=1.2,cex.axis=1.2, col="darkblue",lwd=2)
#' abline(h=0, col="red")
#' plot(t,fcenLRinv(t.fine,t_step,dens.true), type="l",
#'   las=1, ylab="density",cex.lab=1.2,cex.axis=1.2, 
#'   col="darkblue",lwd=2)
